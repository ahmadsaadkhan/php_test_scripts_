...
while ($row = $result->fetch_assoc()) {
	...
	if(!$district) {
		$sql = "INSERT INTO districts (ar_name, tenant) VALUES (?,?)";
		$stmt = $mysqli->prepare($sql);
		$stmt->bind_param('ss', $row['district'], $tenant);
		$stmt->execute();
	...
	if(!$sector) {
		$sql = "INSERT INTO sectors (ar_name, district_id, tenant) VALUES (?,?,?)";
		$stmt = $mysqli->prepare($sql);
		$stmt->bind_param('sis', $row['sector'], $district_id, $tenant);
		$stmt->execute();
	...
	var_dump($sector);
	if(!$sector) {
		$sql = "INSERT INTO streets (ar_name, sector_id, tenant) VALUES (?,?,?)";
		$stmt = $mysqli->prepare($sql);
		$stmt->bind_param('sis', $row['street'], $sector_id, $tenant);
		$stmt->execute();
